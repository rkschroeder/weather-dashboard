...
| `data/weather.db` | SQLite database with `hourly` (includes `humidity`) and `daily` tables (auto-created) |
...
- `fetch_weather(lat, lon)` → returns raw JSON with `hourly` and `daily` keys; hourly fields include `temperature_2m`, `precipitation`, `windspeed_10m`, `winddirection_10m`, `relativehumidity_2m`
